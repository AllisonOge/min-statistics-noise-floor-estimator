...
class NoiseEstimator:
    ...
    def __init__(self, size, initial_noise_amp):
        """initialize constructor"""
        self.__noise_est = 20 * np.log10(initial_noise_amp)
        self.__last_psd_est = 0
        self.__prev_psd_est = np.ones(size) * initial_noise_amp**2
    ...
    def compute(self, lin_fft):
        ...
        print("bias correction factor is ", b_corfac)
        # in each frame
        for n in range(len(lin_fft)):
            ...
            # estimated variance
            est_var = abs(self.__smoment - self.__fmoment**2)
            # print("The estimated variance is ", est_var)
            # equivalent degree of freedom
            eq_deg = 10 * np.log10(0.5 * est_var) - (2 * self.__noise_est) # inverse of equivalent degree of freedom
            # print(10 ** (eq_deg / 10))
            ...
            if n == 0:  # for the first sample in a frame
                noise_range = self.__prev_noise_est[len(lin_fft) - win_min - 1:] + noise_slope_max # get win_len - 1 previous samples
